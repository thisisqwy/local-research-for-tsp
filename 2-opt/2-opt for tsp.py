...
class two_opt(object):
    ...
    def main(self):
        ...
        print('当前最短路径长度为：%.2f' % (1/best_fitness))
        for i in range(self.num_pop):#对初试解的每个解都用2-opt算法优化到无法再优化的地步。
            improved = True
            best=self.pop[i].copy()
            while improved:
                improved = False
                best_distance=self.path_length(best)
                for j in range(1,self.num_city-1):
                    for k in range(j+1,self.num_city):
                        new_path=best[0:j]+best[j:k+1][::-1]+best[k+1:]#如果k+1=num_city，那么best[k+1:]就会等于一个空列表。
        ...
        print('最终的3最短路径长度为：%.2f' % (1/best_fitness))
